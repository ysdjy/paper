...
from runtime.drawer_ik_common import HOME_Q_VERTICAL_RAISED, open_direction_world
...
class CloseDrawerIKConfig:
    ...
    # 预抓取【路点】到位阈值(宽松，只是过渡点)。收太紧会在够不到时超时失败。
    reach_pos_threshold: float = 0.025
    reach_ori_threshold: float = math.radians(18.0)  # also require wrist alignment before grasping
    reach_stable_cycles: int = 6
    soft_reach_advance: float = 0.04   # advance on reach timeout if within this; let physical push engage
    soft_reach_ori: float = math.radians(40.0)  # orientation tolerance for the timeout soft-advance
    # 【闭合夹爪前】收紧：夹爪真正贴到把手方块(半宽 1.8cm)再合，否则夹边缘一推就滑脱。
    grasp_reach_pos_threshold: float = 0.012
    grasp_reach_ori_threshold: float = math.radians(15.0)
    grasp_soft_reach_advance: float = 0.02
...
class CloseDrawerIKSkill:
    ...
    def _grasp_pose(self, lead: float, horizontal_lead: bool = False) -> PoseState:
        ...
        """
        # SINGLE source: position = _handle_pos() (link o handle_offset), orientation = link o handle_quat.
        # Both handle_offset & handle_quat are the UI-adjusted pose (live override injected in start(),
        # else the saved grasp_poses.json pose). No override branch, no computed-quat fallback.
        handle = self._handle_pos()
        eid = self.adapter.env_id
        link_quat = self.obs_adapter.cabinet.data.body_quat_w[eid, self.obs_adapter._link_idx]
        quat = math_utils.quat_mul(link_quat.unsqueeze(0), self.obs_adapter.handle_quat.unsqueeze(0))[0]
    ...
    def start(self, state: SceneState):
        ...
        # SINGLE handle pose: push the live UI grasp pose (override_grasp_local) into the obs_adapter so
        # the grasp target AND the tracked handle share ONE source = the Grasp Pose panel pose.
        ov = (self.request.parameters or {}).get("override_grasp_local")
        if ov is not None:
            self.obs_adapter.set_handle_pose(ov["pos"], ov["quat"])
        robot = self.env.unwrapped.scene["robot"]
        ...
        if (self.request.parameters or {}).get("start_at_approach"):
            ...
            # 关闭常从【已转身+前伸】的开抽屉终态开始：先在【关节空间】ARC_TO_FACE 转身对正抽屉(关节插值绝不
            # 自撞)，再直接去把手(就在刚松开的开抽屉处、近距离)。不做任务空间 PRELIFT 抬升(姿态未转正时任务
            # 空间直线运动会夹爪撞本体)，也不整臂回 home(桌面柜会够不到 -> reach 超时)。
            initial_state = "ARC_TO_FACE"
        elif self.cfg.use_turn_to_face and self.cfg.start_from_current:
    ...
    def _home_step(self, state: SceneState) -> SkillCommand:
        ...
        if dist <= self.cfg.home_joint_threshold:
            ...
            if self.runtime.stable_count >= 3:
                self._transition(state, self._post_prelift_state())   # 回 home 后 -> ARC_TO_FACE 转身再抓
        else:
    ...
    def step(self, state: SceneState, dt: float) -> SkillCommand:
        ...
        if self.runtime.state == "PRELIFT":
            ...
            gripper = 1.0
            # 闭合前收紧到位判定：夹爪真正贴到方块(1.2cm)再合，避免没夹住就推。
            self._advance_when_reached(state, gp, "CLOSE_GRIPPER", self.cfg.reach_timeout,
                                       pos_thresh=self.cfg.grasp_reach_pos_threshold,
                                       ori_thresh=self.cfg.grasp_reach_ori_threshold,
                                       soft_pos=self.cfg.grasp_soft_reach_advance)
            ...
            # 水平推入：沿【抓取 pose 自己的接近轴】(lead<0 = 朝柜内)，对桌面柜和地面 Sektion 柜都成立
            # (各自把手 pose 已朝向各自抽屉);不能用 open_direction_world(桌面柜专属轴，对 Sektion 方向错)。
            target = self._grasp_pose(-self.cfg.push_lead, horizontal_lead=True)
            gripper = -1.0
    ...
    def viz_poses(self) -> list:
        ...
        try:
            # marker uses the ACTUAL grasp orientation (= UI pose), matching where the gripper grasps.
            gp = self._grasp_pose(0.0)
            out.append(("drawer_handle", PoseState(gp.pos_w, gp.quat_w)))
        except Exception:
            pass
        return out
    ...
    def _advance_when_reached(self, state: SceneState, target: PoseState, next_state: str, timeout: float,
                              pos_thresh: float | None = None, ori_thresh: float | None = None,
                              soft_pos: float | None = None) -> bool:
        pos_thresh = self.cfg.reach_pos_threshold if pos_thresh is None else pos_thresh
        ori_thresh = self.cfg.reach_ori_threshold if ori_thresh is None else ori_thresh
        soft_pos = self.cfg.soft_reach_advance if soft_pos is None else soft_pos
        err = pose_error(state.robot.tcp_pose, target)
        if err.position <= pos_thresh and err.orientation <= ori_thresh:
            self.runtime.stable_count += 1
        ...
        if self._state_elapsed(state) > timeout:
            if err.position <= soft_pos and err.orientation <= self.cfg.soft_reach_ori:
