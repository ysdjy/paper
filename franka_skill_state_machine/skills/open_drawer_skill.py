...
class OpenDrawerIKSkill:
    ...
    def _grasp_pose(self, lead: float, horizontal_lead: bool = False) -> PoseState:
        ...
        """
        # SINGLE source: obs_adapter.handle_offset (pos) + handle_quat (quat), both link-local, are the
        # UI-adjusted pose (live override injected in start(), else the saved grasp_poses.json pose). The
        # grasp/pre-grasp/pull target == EXACTLY that pose. No override branch, no computed-quat fallback.
        eid = self.adapter.env_id
        ...
        hq = self.obs_adapter.handle_quat   # UI orientation (link-local); guaranteed by obs_adapter init
        gpos, gquat = math_utils.combine_frame_transforms(
            link_pos.unsqueeze(0), link_quat.unsqueeze(0), total_local.unsqueeze(0), hq.unsqueeze(0))
        quat = gquat[0]   # EXACT UI orientation
    ...
    def start(self, state: SceneState):
        ...
        # SINGLE handle pose: if the request carries the live UI grasp pose (override_grasp_local), push it
        # INTO the obs_adapter so the grasp target AND the tracked handle (pull/push/detach/azimuth/viz)
        # all read the SAME pose = the one set in the Grasp Pose panel. No separate override code path.
    ...
    def step(self, state: SceneState, dt: float) -> SkillCommand:
        ...
        if self.runtime.state == "PRELIFT":
            ...
            gripper = 1.0
            # 闭合前收紧到位判定：夹爪真正贴到方块(1.2cm)再合，避免没夹住就拉。
            ...
            # 水平后拉：沿【抓取 pose 自己的接近轴】(-Z 投影到水平)，即你为该柜子设的把手朝向所定义的拉出
            # 方向。这对桌面柜和地面 Sektion 柜都成立(各自把手 pose 已朝向各自抽屉);不能用 open_direction_world
            # (那用的是桌面柜专属的 LOCAL_OPEN_AXIS，对 Sektion 方向全错、抽屉纹丝不动)。
            target = self._grasp_pose(self._eff["pull_lead"], horizontal_lead=True)
            gripper = -1.0
    ...
    def viz_poses(self) -> list:
        ...
        try:
            # draw the handle marker with the ACTUAL grasp orientation (= UI pose), so the visualized
            # frame matches where the gripper grasps -- no phantom Y-up frame from a computed quat.
