...
class _WaypointMover:
    ...
    def __init__(self, adapter, waypoints, *, device, pos_tol: float = 0.05):
        ...
        self._t = 0.0
        self.done = False
        self._joint_cmd = None                  # 关节途径点:限速平滑逼近用的命令构型
    ...
    def step(self, state, dt):
        ...
        if kind == "joint":
            # 到位判定看【关节1转到位】(转身的关键,≈ARC_TO_FACE),不苛求整臂 L2——否则大转身常超时切走、
            # 底座没转够。超时放宽到 12s。
            j1_err = abs(float(state.robot.joint_pos[self.adapter._joint_ids][0]) - float(data[0]))
            reached = j1_err < 0.14
            timeout = self._t > 12.0
        ...
        if reached or timeout:
            ...
            kind, data, quat, rad = self.wps[self.i]
        if kind == "joint":
            # 限速平滑逼近目标构型(每关节每帧≤0.05rad),像 ARC_TO_FACE 那样柔和转身,不整臂大跳/甩臂。
            if self._joint_cmd is None:
                self._joint_cmd = state.robot.joint_pos[self.adapter._joint_ids].clone()
            self._joint_cmd = self._joint_cmd + torch.clamp(data - self._joint_cmd, -0.05, 0.05)
            return self._joint_cmd.clone(), 1.0
...
class SkillTestController:
    ...
    def _on_operate_coffee(self):
        """抓住咖啡机把手方块，绕【竖直世界 Z 轴】(把手关节 joint_5 的旋转轴,水平摆动)闭环旋转，把
        joint_5 从当前角物理拖到目标角。轴固定为世界 Z(不再做脆弱/易超时的探测标定)；旋转中心取该关节的
        世界锚点(读不到则回退 lever link 原点)。闭环 joint_reader->joint_target 会自动修正微小偏差。"""
        ...
        self._grasp_runner = None
        self._coffee_calib = None
        jn = os.environ.get("SKILL_TEST_COFFEE_JOINT", COFFEE_LEVER_JOINT)   # joint_5：绕世界 Z 水平摆动
        asset = self.provider.scene["coffee_machine"]
        axis = torch.tensor([0.0, 0.0, 1.0], device=self.device)            # 竖直世界 Z 轴(用户确认)
        anchor = None
        try:
            a, _ = self._revolute_joint_axis_world("coffee_machine", jn, COFFEE_LEVER_LINK)
            if a is not None:
                anchor = a.clone()                                          # 关节世界锚点=旋转中心 x,y
        except Exception as exc:
            print(f"[SkillTestController] coffee anchor read failed ({exc}); fallback link origin", flush=True)
        if anchor is None:
            ci = next((k for k, nn in enumerate(list(asset.data.body_names)) if nn == COFFEE_LEVER_LINK), None)
            anchor = asset.data.body_pos_w[0, ci].clone() if ci is not None else pose_w.pos_w.clone()
        self._coffee_pose = pose_w
        self._coffee_target = float(os.environ.get("SKILL_TEST_COFFEE_TARGET", "0.55"))
        self._free_joint("coffee_machine", jn)                              # 松成自由关节，让机器人物理拖动
        ids, _ = asset.find_joints(jn)

        def _read_joint():
            return float(asset.data.joint_pos[0, ids[0]])
        self._grasp_runner = _GraspPoseRunner(
            self.adapter, pose_w, device=self.device, speed=self._runner_speed,
            arrival_z=None, retreat=False, standoff=0.06,
            sweep_hinge=anchor, sweep_axis=axis,
            joint_reader=_read_joint, joint_target=self._coffee_target)
        print(f"[SkillTestController] coffee: grasp lever + sweep {jn} about WORLD-Z "
              f"anchor=({float(anchor[0]):.3f},{float(anchor[1]):.3f},{float(anchor[2]):.3f}) "
              f"target={self._coffee_target:.2f}", flush=True)
        self._set_status("coffee: grasp + sweep about world-Z")
    ...
    # --------------------------------------------------------------- autorun
    def _dump_handle_axes(self, state):
        """诊断: 打印每个抽屉把手 grasp pose 的【世界 Y 轴】及其与竖直的夹角(应≈0°=Y 严格垂直地面)。
        world_grasp_quat = quat_mul(link_quat_w, saved_local_quat); Y_world = quat_apply(quat, +Y)。
        由 SKILL_TEST_DUMP_HANDLES=1 触发,autorun 场景 live 后跑一次。"""
        import math
        import isaaclab.utils.math as mu
        import torch
        poses = self._saved_grasp_poses or self._load_grasp_poses()
        try:
            cab = self.provider.scene["cabinet"]
        except Exception:
            print("[DUMP] no cabinet in scene", flush=True)
            return
        dev = cab.data.body_quat_w.device
        ey = torch.tensor([[0.0, 1.0, 0.0]], device=dev)
        ez = torch.tensor([[0.0, 0.0, 1.0]], device=dev)
        print("[DUMP] drawer handle grasp-pose Y axis (world) -- Y should be vertical (0,0,±1):", flush=True)
        for name in ("handle_top_drawer", "handle_middle_drawer", "handle_bottom_drawer"):
            e = poses.get(name)
            if e is None or e.get("member") != "cabinet":
                print(f"[DUMP]   {name}: MISSING", flush=True)
                continue
            li = self._link_idx(cab, e["link"])
            if li is None:
                print(f"[DUMP]   {name}: link {e['link']} not found", flush=True)
                continue
            lq = cab.data.body_quat_w[0, li].reshape(1, 4)
            loc = torch.tensor([e["quat"]], device=dev, dtype=lq.dtype)
            wq = mu.quat_mul(lq, loc)
            yw = mu.quat_apply(wq, ey)[0]
            zw = mu.quat_apply(wq, ez)[0]
            yv = float(abs(yw[2]) / (torch.linalg.norm(yw) + 1e-9))
            ang = math.degrees(math.acos(max(-1.0, min(1.0, yv))))       # angle of Y from world vertical
            zhz = math.degrees(math.acos(max(-1.0, min(1.0, float(abs(zw[2]) / (torch.linalg.norm(zw) + 1e-9))))))
            zdev = 90.0 - zhz                                             # approach(+Z) deviation from horizontal
            print(f"[DUMP]   {name}: Y_world=({yw[0]:+.3f},{yw[1]:+.3f},{yw[2]:+.3f}) "
                  f"Y-from-vertical={ang:5.2f}deg  approach(+Z)-off-horizontal={zdev:5.2f}deg "
                  f"local_quat={[round(v,4) for v in e['quat']]}", flush=True)

    def _auto_step(self, state):
        if os.environ.get("SKILL_TEST_DUMP_HANDLES") == "1" and not getattr(self, "_handles_dumped", False):
            self._handles_dumped = True
            try:
                self._dump_handle_axes(state)
            except Exception as exc:
                import traceback
                traceback.print_exc()
