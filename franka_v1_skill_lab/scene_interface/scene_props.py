    ...
    try:
        ...
        if rigid:
            ...
            spawn.rigid_props = sim_utils.RigidBodyPropertiesCfg(
                max_depenetration_velocity=3.0,   # was 0.5: resolve grasp overlaps fast -> no visible 穿模
                solver_position_iteration_count=16,
                solver_velocity_iteration_count=1,
            )
            # 接触在视觉面之前一点点触发(rest_offset>0) -> 夹取时夹爪不再穿进物体;碰撞体几何不变、仍==绿色可视。
            spawn.collision_props = sim_utils.CollisionPropertiesCfg(rest_offset=0.002, contact_offset=0.02)
            cfg = RigidObjectCfg(
                prim_path="{ENV_REGEX_NS}/" + name,
...
def _install_grasp_block(stage, link_path: str, local_pos, half=(0.018, 0.018, 0.030),
                         friction: float = 2.2, name: str = "GraspBlock") -> bool:
...
_GRASP_BLOCK_SPECS = [
    {"hname": "handle_top_drawer",    "member": "cabinet",        "prim": "Cabinet",       "link": "link_0", "half": (0.018, 0.018, 0.030), "friction": 2.2},
    {"hname": "handle_middle_drawer", "member": "cabinet",        "prim": "Cabinet",       "link": "link_2", "half": (0.018, 0.018, 0.030), "friction": 2.2},
    {"hname": "handle_bottom_drawer", "member": "cabinet",        "prim": "Cabinet",       "link": "link_1", "half": (0.018, 0.018, 0.030), "friction": 2.2},
