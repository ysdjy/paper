...
class SelectedDrawerObsAdapter:
    ...
    def __init__(self, env, target_drawer: str, env_id: int = 0):
        ...
        # auto-located bar center sits a bit OUTSIDE the graspable point -> pull the target inward (toward
        # the cabinet, along -open_dir) so the fingers actually wrap the bar. Tunable via env var.
        import os as _os
        self._handle_inward = float(_os.environ.get("SKILL_TEST_DRAWER_HANDLE_INWARD", "0.025"))
        ...
        self.last_obs_dim: int | None = None
        # Handle source priority (user wants the grasp target == the pose THEY set in scene editing, no
        # auto-located/new pose): 1) saved grasp_poses.json handle_<drawer> link-local pos; else
        # 2) proxy prim; else 3) mesh auto-locate; else 4) config offset.
        self.handle_quat = None       # saved grasp orientation (link-local), if any
    ...
    def _calibrate_handle_offset_from_proxy(self) -> None:
        position = self.target_drawer.split("_")[0]                 # top / middle / bottom
        proxy_name = position.capitalize() + "HandleProxy"          # e.g. MiddleHandleProxy
        try:
            from pxr import Usd, UsdGeom

            stage = self.env.unwrapped.sim.stage
            proxy_path = f"/World/envs/env_{self.env_id}/Cabinet/{self.link_name}/{proxy_name}"
            prim = stage.GetPrimAtPath(proxy_path)
            if not prim.IsValid():
                print(f"[SelectedDrawerObsAdapter] handle proxy not found ({proxy_path}); "
                      "trying mesh auto-locate", flush=True)
                self._auto_locate_handle_from_meshes()
                return
            t = UsdGeom.XformCache(Usd.TimeCode.Default()).GetLocalToWorldTransform(prim).ExtractTranslation()
            proxy_w = torch.tensor([float(t[0]), float(t[1]), float(t[2])],
                                   dtype=torch.float32, device=self.device)
            link_pos = self.cabinet.data.body_pos_w[self.env_id, self._link_idx]
            link_quat = self.cabinet.data.body_quat_w[self.env_id, self._link_idx]
            ...
            print(f"[SelectedDrawerObsAdapter] {self.target_drawer} handle offset calibrated from {proxy_name}: "
                  f"config={old} -> actual={new}", flush=True)
        except Exception as exc:  # pragma: no cover
            print(f"[SelectedDrawerObsAdapter] handle proxy calibration failed ({exc}); using config offset",
                  flush=True)

    def _auto_locate_handle_from_meshes(self) -> None:
        """Find the drawer handle from the link's child meshes: the thin mesh that protrudes MOST in the
        drawer's open direction (the bar sticks out the front past the panel). Back out the link-local
        offset from its world center. Mirrors DoorIKSkill._auto_locate_handle_from_meshes."""
        try:
            from pxr import Usd, UsdGeom

            from runtime.drawer_ik_common import open_direction_world

            stage = self.env.unwrapped.sim.stage
            link_path = f"/World/envs/env_{self.env_id}/Cabinet/{self.link_name}"
            prim = stage.GetPrimAtPath(link_path)
            if not prim.IsValid():
                print(f"[SelectedDrawerObsAdapter] link prim not found ({link_path}); using config offset",
                      flush=True)
                return
            bb = UsdGeom.BBoxCache(Usd.TimeCode.Default(),
                                   [UsdGeom.Tokens.default_, UsdGeom.Tokens.render], useExtentsHint=True)
            meshes = []   # (center xyz, size xyz, volume)
            ...
            if not meshes:
                print("[SelectedDrawerObsAdapter] no meshes under drawer link; using config offset", flush=True)
                return
            link_pos = self.cabinet.data.body_pos_w[self.env_id, self._link_idx]
            link_quat = self.cabinet.data.body_quat_w[self.env_id, self._link_idx]
            od = open_direction_world(self.cabinet.data.root_quat_w[self.env_id])   # world open dir (unit, 3,)
            slab_vol = max(m[2] for m in meshes)
            cand = [m for m in meshes if m[2] < slab_vol and max(m[1]) > 0.03] or meshes
            for ctr, size, vol in sorted(meshes, key=lambda m: m[2]):
                fwd = float(torch.dot(torch.tensor([float(ctr[0]), float(ctr[1]), float(ctr[2])],
                                                   device=self.device) - link_pos, od))
                print(f"[SelectedDrawerObsAdapter]   mesh ctr=({ctr[0]:.3f},{ctr[1]:.3f},{ctr[2]:.3f}) "
                      f"size=({size[0]:.3f},{size[1]:.3f},{size[2]:.3f}) fwd={fwd:.3f}", flush=True)

            def fwd_of(m):
                c = torch.tensor([float(m[0][0]), float(m[0][1]), float(m[0][2])], device=self.device)
                return float(torch.dot(c - link_pos, od))

            handle = max(cand, key=fwd_of)                            # most-forward thin mesh = the bar
            ctr = handle[0]
            handle_w = torch.tensor([float(ctr[0]), float(ctr[1]), float(ctr[2])],
                                    dtype=torch.float32, device=self.device)
            handle_w = handle_w - od * self._handle_inward            # 往里收一点，确保夹住把手
            off, _ = math_utils.subtract_frame_transforms(
                link_pos.unsqueeze(0), link_quat.unsqueeze(0), handle_w.unsqueeze(0))
            old = [round(float(v), 4) for v in self.handle_offset.tolist()]
            self.handle_offset = off[0].clone()
            new = [round(float(v), 4) for v in self.handle_offset.tolist()]
            print(f"[SelectedDrawerObsAdapter] {self.target_drawer} handle auto-located: bar_center="
                  f"({float(ctr[0]):.3f},{float(ctr[1]):.3f},{float(ctr[2]):.3f}) inward={self._handle_inward:.3f} "
                  f"config_offset={old} -> actual={new}", flush=True)
        except Exception as exc:  # pragma: no cover
            print(f"[SelectedDrawerObsAdapter] mesh auto-locate failed ({exc}); using config offset", flush=True)
